...
export async function seedPlayer(userId, name) {
  ...
  await Promise.all([
    ...
    ...seedBooks.books.map(
      (b, i) => sql`
        INSERT INTO books (user_id, position, title, authors, area, phase, priority, free_url, chapters_total, chapters_read, why)
        VALUES (${userId}, ${i}, ${b.title}, ${b.authors}, ${b.area}, ${String(b.phase)}, ${b.priority}, ${b.free}, ${b.chaptersTotal}, 0, ${b.why})
      `
    ),
    sql`
