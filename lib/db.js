...
export async function getPlayerData(userId) {
  const [players, phaseRows, msRows, courseRows, bookRows, logRows, shadowRows] = await Promise.all([
    sql`SELECT user_id, name, job, awakened::text, cached_xp, cached_level, alloc,
               on_leaderboard, public_name FROM players WHERE user_id = ${userId}`,
    sql`SELECT id, position, name, window_label, status, goal FROM phases WHERE user_id = ${userId} ORDER BY position`,
    sql`SELECT m.id, m.phase_id, m.position, m.text, m.done, m.proof_url, m.proof_verified FROM milestones m
    ...
    sql`SELECT id, name, role, url, phase_position, extracted::text FROM shadows
        WHERE user_id = ${userId} ORDER BY phase_position NULLS LAST, id`,
  ]);
  ...
  const phases = phaseRows.map((p) => ({
    ...
    milestones: msRows
      ...
      .map((m) => ({
        ...
        proof: m.proof_url,
        proofVerified: m.proof_verified,
      })),
  }));
  ...
  return {
    player: {
      ...players[0],
      alloc: players[0].alloc || {},
      cachedLevel: players[0].cached_level,
      onLeaderboard: players[0].on_leaderboard,
      publicName: players[0].public_name,
    },
    roadmap: { phases },
    courses: {
    ...
    shadows: shadowRows.map((s) => ({
      dbId: s.id,
      name: s.name,
      role: s.role,
      url: s.url,
      phase: s.phase_position,
