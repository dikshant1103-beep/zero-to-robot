...
export async function addCourse(f) {
  ...
  await sql`
    ...
    VALUES (
      ...
      ${clampInt(f.hours, 1, 500, 10)}, ${String(f.url || "").slice(0, 500)}
    )
  `;
  refresh();
}
...
export async function addBook(f) {
  ...
  await sql`
    ...
    VALUES (
      ...
      ${f.free ? String(f.free).slice(0, 500) : null},
...
export async function updateBook(id, f) {
  ...
  await sql`
    UPDATE books SET
      ...
      free_url = ${f.free ? String(f.free).slice(0, 500) : null},
...
export async function addLogEntry(f) {
  ...
  await sql`
    ...
    VALUES (${userId}, ${date}, ${clampInt(f.phase, 0, 20, 0)}, ${title}, ${String(f.body || "").slice(0, 5000)})
  `;
  refresh();
}
